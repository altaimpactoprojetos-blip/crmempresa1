...
Object.assign(CRM, {
  ...
  unread: 0,
  isManager() {
    return this.user && (this.user.role === 'admin' || this.user.role === 'supervisor');
  },
  isAdmin() {
...
CRM.onUnauthorized = () => {
  if (CRM.user) {
    CRM.user = null;
    UI.toast('Sua sessão expirou. Faça login novamente.', 'warning');
    renderAuth();
  }
};
...
function brandHtml(cls = '') {
  ...
  const logo = s.logo_data
    ? `<img src="${UI.attr(s.logo_data)}" alt="">`
    : `<span class="logo-fallback">${UI.esc(UI.initials(s.name || 'CRM'))}</span>`;
    ...
    try {
      if (view === 'login') {
        ...
        if (!r.mail_configured)
          UI.toast(
            'Envio de e-mail não configurado neste servidor: peça ao administrador um link de redefinição.',
            'warning',
            9000,
          );
      } else {
        if (d.password !== d.confirm) {
          UI.showErrors(form, new ApiError(400, { fields: { confirm: 'As senhas não coincidem.' } }));
          btn.disabled = false;
          return;
        }
        const r = await api('/auth/reset-password', { method: 'POST', body: { token: param, password: d.password } });
        UI.ok(r.message);
        location.hash = '#/login';
      }
    } catch (err) {
...
function renderShell() {
  ...
  if (av)
    av.onchange = async () => {
      try {
        const r = await api('/auth/me/availability', { method: 'PUT', body: { available: av.checked } });
        CRM.user.available = r.available;
        root.querySelector('#availDot').className = `dot ${r.available ? 'on' : 'off'}`;
        UI.ok(
          r.available
            ? 'Você está disponível para novos atendimentos.'
            : 'Você está indisponível para distribuição automática.',
        );
      } catch (e) {
        UI.err(e);
      }
...
async function refreshBadges() {
  try {
    ...
    CRM.unread = n.unread;
    const c = root.querySelector('#notifCount');
    if (c) {
...
function setBadge(nav, n, cls) {
  const el = root.querySelector(`[data-nav-badge="${nav}"]`);
  if (!el) return;
  el.textContent = n;
...
async function toggleNotifications() {
  ...
  const alerts = [];
  if (r.alerts.overdue_tasks)
    alerts.push(
      `<div class="item unread"><strong>${r.alerts.overdue_tasks} tarefa(s) atrasada(s)</strong><br><a href="#/tarefas">Ver tarefas</a></div>`,
    );
  if (r.alerts.follow_ups_due)
    alerts.push(
      `<div class="item unread"><strong>${r.alerts.follow_ups_due} retorno(s) pendente(s) até amanhã</strong><br><a href="#/atendimentos?follow_up=pending">Ver atendimentos</a></div>`,
    );
  ...
  panel.querySelector('#readAll').onclick = async () => {
    await api('/notifications/read-all', { method: 'POST' });
    panel.remove();
    refreshBadges();
  };
  panel.addEventListener('click', (e) => {
    if (e.target.closest('a')) setTimeout(() => panel.remove(), 50);
  });
...
function connectRealtime() {
  ...
  const refresh = UI.debounce(() => {
    refreshBadges();
    if (CRM.currentPage && CRM.currentPage.onRealtime) CRM.currentPage.onRealtime();
  }, 400);
  ...
  es.addEventListener('notification', (e) => {
    ...
    UI.toast(`${d.title}${d.body ? ': ' + d.body : ''}`, 'info', 6000);
    refresh();
  });
  es.onerror = () => {
    es.close();
    setTimeout(() => {
      if (CRM.user) connectRealtime();
    }, 5000);
  };
}
...
function setupSearch() {
  const input = root.querySelector('#globalSearch'),
    box = root.querySelector('#searchResults');
  ...
  input.onkeydown = (e) => {
    ...
    if (e.key === 'Enter') {
      location.hash = `#/clientes?q=${encodeURIComponent(input.value.trim())}`;
      box.hidden = true;
    }
  };
  box.onclick = () => {
...
CRM.pages.profile = {
  async render(el) {
    el.innerHTML =
      CRM.pageHeader('Meu perfil', `${CRM.user.name} · ${CRM.user.email}`) +
      `<div class="grid cols-2"><div class="card"><h3>Alterar senha</h3>
    ...
    el.querySelector('#pwForm').onsubmit = async (e) => {
      e.preventDefault();
      try {
        const r = await api('/auth/me/password', { method: 'PUT', body: UI.formData(e.target) });
        UI.ok(r.message);
        e.target.reset();
      } catch (err) {
