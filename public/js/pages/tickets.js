...
CRM.pages.tickets = {
  async render(el, { id, query }) {
    ...
    el.innerHTML =
      CRM.pageHeader(
        'Atendimentos',
        'Central de atendimentos: fila de espera, andamento e histórico.',
    ...
    el.querySelector('#tabs').onclick = (e) => {
      ...
      if (!b) return;
      location.hash = `#/atendimentos?tab=${b.dataset.tab}`;
    };
    el.querySelector('#filters').onsubmit = (e) => {
      ...
      location.hash = `#/atendimentos?${qs}`;
    };
    el.querySelector('#btnNew').onclick = () => this.form({}, () => this.list());
    el.querySelector('#btnExport').onclick = () =>
      UI.download(`/tickets/export.csv?from=${query.from || ''}&to=${query.to || ''}`);
    const dist = el.querySelector('#btnDistribute');
    if (dist)
      dist.onclick = async () => {
        try {
          const r = await api('/tickets/distribute', { method: 'POST' });
          UI.toast(r.message, r.assigned ? 'success' : 'warning');
          this.list();
        } catch (err) {
          UI.err(err);
        }
    ...
    await this.list();
    if (query.novo) {
      history.replaceState(null, '', '#/atendimentos');
      this.form({}, () => this.list());
    }
  },
  ...
  async list() {
    ...
    if (this.tab === 'queue') q.status = 'aguardando';
      ...
      q.mine = 'true';
      q.status = this.query.status || 'aguardando,em_atendimento,aguardando_cliente';
    } else {
      q.status = this.query.status;
      q.assignee_id = this.query.assignee_id;
      if (this.query.follow_up) q.follow_up = this.query.follow_up;
    }
    const r = await api('/tickets', { query: q });
    if (!r.tickets.length) {
      box.innerHTML = UI.empty(
        this.tab === 'queue' ? 'Fila vazia' : 'Nenhum atendimento encontrado',
        this.tab === 'queue'
          ? 'Todos os atendimentos foram assumidos.'
          : 'Ajuste os filtros ou abra um novo atendimento.',
      );
      return;
    }
    ...
    box.querySelectorAll('tr[data-id]').forEach(
      (tr) =>
        (tr.onclick = (e) => {
          ...
          location.hash = `#/atendimentos/${tr.dataset.id}`;
        }),
    );
    box
  ...
  async claim(id, after) {
    try {
      const r = await api(`/tickets/${id}/claim`, { method: 'POST' });
      UI.ok(r.message);
      if (after) after();
      else location.hash = `#/atendimentos/${id}`;
    } catch (err) {
      UI.toast(err.message, 'warning', 6000);
      if (after) after();
    }
  },
  ...
  async form({ customer } = {}, onSaved) {
    ...
    const assigneeOpts = CRM.isManager()
      ? UI.userOptions(CRM.users, { blank: '— Deixar na fila —' })
      : [
          ['', '— Deixar na fila —'],
          [CRM.user.id, `${CRM.user.name} (eu)`],
        ];
    const m = UI.modal({
      ...
      <div class="form-row cols-3">${UI.field(
        'channel',
        'Canal de origem',
        UI.select(
          'channel',
          this.channels.map((c) => [c, c]),
          this.channels[0],
        ),
        { required: true },
      )}
        ${UI.field(
          ...
          UI.select(
            'priority',
            Object.entries(UI.PRIORITY).map(([k, v]) => [k, v.label]),
            'normal',
          ),
        )}
    ...
    if (!customer)
      this.customerPicker(form.querySelector('#custSearch'), form.querySelector('#custResults'), form.customer_id, m);
    form.onsubmit = async (e) => {
      e.preventDefault();
      const d = UI.formData(form);
      if (!d.customer_id) {
        UI.showErrors(form, new ApiError(400, { fields: { customer_id: 'Selecione um cliente.' } }));
        return;
      }
      try {
        const r = await api('/tickets', { method: 'POST', body: d });
        UI.ok(r.message);
        m.close();
        if (onSaved) onSaved(r.ticket);
        location.hash = `#/atendimentos/${r.ticket.id}`;
      } catch (err) {
  ...
  customerPicker(input, box, hidden, modal) {
    const run = UI.debounce(async () => {
      ...
      box.innerHTML = r.customers.length
        ? r.customers
            .map(
              (c) =>
                `<a href="#" data-id="${c.id}" data-name="${UI.attr(c.name)}"><strong>${UI.esc(c.name)}</strong> <span class="muted small">${UI.esc(c.phone || '')} ${UI.esc(c.company || '')}</span></a>`,
            )
            .join('')
        : '<div class="cat">Nenhum cliente encontrado</div>';
      box.hidden = false;
    }, 250);
    input.oninput = () => {
      hidden.value = '';
    ...
    const nb = modal.el.querySelector('#newCust');
    if (nb)
      nb.onclick = () =>
        CRM.pages.customers.form(null, (c) => {
          hidden.value = c.id;
          input.value = c.name;
        });
  },
  ...
  async detail(el, id) {
    ...
    const isOwner = t.assignee_id === CRM.user.id,
      mgr = CRM.isManager();
    const canAct = open && (isOwner || mgr);
    const phone = UI.digits(t.customer_phone);
    const wa = phone ? UI.waLink(phone.length <= 11 ? '55' + phone : phone) : null;
    const actions = [];
    if (t.status === 'aguardando' && (!t.assignee_id || isOwner))
      actions.push('<button class="btn success" data-act="claim">Assumir atendimento</button>');
    if (t.status === 'aguardando' && t.assignee_id && !isOwner && mgr)
      actions.push('<button class="btn secondary" data-act="transfer">Reatribuir</button>');
    if (canAct && t.status !== 'aguardando') {
      if (t.status !== 'aguardando_cliente')
        actions.push(
          '<button class="btn secondary" data-act="status" data-status="aguardando_cliente">Aguardando cliente</button>',
        );
      else
        actions.push(
          '<button class="btn secondary" data-act="status" data-status="em_atendimento">Retomar atendimento</button>',
        );
      actions.push(
        '<button class="btn secondary" data-act="transfer">Transferir</button>',
        '<button class="btn secondary" data-act="release">Devolver à fila</button>',
        '<button class="btn secondary" data-act="followup">Agendar retorno</button>',
        '<button class="btn" data-act="status" data-status="resolvido">Resolver</button>',
        '<button class="btn danger" data-act="status" data-status="cancelado">Cancelar</button>',
      );
    } else if (canAct && t.status === 'aguardando' && mgr)
      actions.push('<button class="btn danger" data-act="status" data-status="cancelado">Cancelar</button>');
    if (!open) actions.push('<button class="btn" data-act="reopen">Reabrir</button>');
    if (open && (isOwner || mgr || !t.assignee_id))
      actions.push('<button class="btn ghost" data-act="edit">Editar</button>');
    ...
    <div class="grid" style="grid-template-columns: 1fr 340px">
      <div class="stack">
        <div class="card"><h3>Registrar</h3>
          ...
          <form id="regForm">
            <div class="form-row" id="interFields">${UI.field(
              'direction',
              'Direção',
              UI.select('direction', [
                ['saida', 'Saída (resposta ao cliente)'],
                ['entrada', 'Entrada (cliente entrou em contato)'],
              ]),
            )}${UI.field('channel', 'Canal', UI.select('channel', (this.channels || [t.channel]).includes(t.channel) ? (this.channels || [t.channel]).map((c) => [c, c]) : [[t.channel, t.channel], ...(this.channels || []).map((c) => [c, c])], t.channel))}</div>
    ...
    el.querySelector('#regTabs').onclick = (e) => {
      const b = e.target.closest('button');
      if (!b) return;
      kind = b.dataset.kind;
      el.querySelectorAll('#regTabs button').forEach((x) => x.classList.toggle('active', x === b));
      el.querySelector('#interFields').hidden = kind !== 'interaction';
      el.querySelector('#regHint').textContent =
        kind === 'note'
          ? 'Anotações internas não são visíveis ao cliente e não contam como resposta.'
          : canAct
            ? 'A primeira interação de saída define o tempo de primeira resposta.'
            : 'Assuma o atendimento para registrar interações.';
      el.querySelector('#regForm button').disabled = kind === 'interaction' && !(canAct || mgr);
    };
    el.querySelector('#regForm').onsubmit = async (e) => {
      e.preventDefault();
      const d = UI.formData(e.target);
      try {
        const r2 =
          kind === 'note'
            ? await api(`/tickets/${id}/notes`, { method: 'POST', body: { body: d.body } })
            : await api(`/tickets/${id}/interactions`, { method: 'POST', body: d });
        UI.ok(r2.message);
        this.detail(el, id);
      } catch (err) {
    ...
    el.querySelector('#btnOpp').onclick = () =>
      CRM.pages.pipeline.form({ customer: { id: t.customer_id, name: t.customer_name }, ticket_id: t.id }, () =>
        this.detail(el, id),
      );
    el.querySelector('#btnTask').onclick = () =>
      CRM.pages.tasks.form(
        { customer_id: t.customer_id, customer_name: t.customer_name, ticket_id: t.id, assignee_id: t.assignee_id },
        () => this.detail(el, id),
      );
    el.querySelectorAll('[data-act]').forEach(
      (b) =>
        (b.onclick = (e) => {
          e.preventDefault();
          this.action(b.dataset.act, t, b.dataset.status, () => this.detail(el, id));
        }),
    );
  },
  ...
  async action(act, t, status, done) {
    try {
      ...
      if (act === 'release') {
        if (!(await UI.confirm('Devolver este atendimento à fila de espera?'))) return;
        const r = await api(`/tickets/${t.id}/release`, { method: 'POST' });
        UI.ok(r.message);
        return done();
      }
      if (act === 'reopen') {
        const note = await UI.prompt('Motivo da reabertura (opcional)', {
          title: 'Reabrir atendimento',
          required: false,
        });
        if (note === null) return;
        const r = await api(`/tickets/${t.id}/reopen`, { method: 'POST', body: { note: note || undefined } });
        UI.ok(r.message);
        return done();
      }
      if (act === 'status') {
        const label = UI.STATUS[status].label;
        let note;
        if (status === 'resolvido' || status === 'cancelado') {
          note = await UI.prompt(
            `${status === 'resolvido' ? 'Resumo da solução' : 'Motivo do cancelamento'} (opcional)`,
            { title: `Marcar como ${label}`, required: false, multiline: true },
          );
          if (note === null) return;
        }
        const r = await api(`/tickets/${t.id}/status`, {
          method: 'POST',
          body: { status, note: note || undefined, version: t.version },
        });
        UI.ok(r.message);
        return done();
      }
        ...
        const m = UI.modal({
          ...
          body: `<form id="trForm">${UI.field(
            ...
            UI.select(
              ...
              opts.map((u) => [
                u.id,
                `${u.name} (${UI.ROLE[u.role]})${u.role === 'atendente' && !u.available ? ' — indisponível' : ''}`,
              ]),
              '',
              'data-type="int" required',
            ),
            { required: true },
          )}${UI.field('reason', 'Motivo', UI.input('reason', '', 'placeholder="Opcional"'))}<input type="hidden" name="version" value="${t.version}" data-type="int"></form>`,
          footer: `<button class="btn secondary" data-close>Cancelar</button><button class="btn" type="submit" form="trForm">Transferir</button>`,
        });
        m.el.querySelector('#trForm').onsubmit = async (e) => {
          e.preventDefault();
          try {
            const r = await api(`/tickets/${t.id}/transfer`, { method: 'POST', body: UI.formData(e.target) });
            UI.ok(r.message);
            m.close();
            done();
          } catch (err) {
      ...
      if (act === 'followup') {
        const m = UI.modal({
          ...
          size: 'narrow',
          body: `<form id="fuForm">${UI.field('at', 'Data e hora do retorno', `<input type="datetime-local" name="at" value="${UI.toLocalInput(t.follow_up_at)}" required>`, { required: true, hint: 'Uma tarefa de retorno será criada para o responsável.' })}${UI.field('note', 'Observação', UI.input('note', '', 'placeholder="Opcional"'))}</form>`,
        ...
        m.el.querySelector('#fuForm').onsubmit = async (e) => {
          e.preventDefault();
          try {
            const d = UI.formData(e.target);
            const r = await api(`/tickets/${t.id}/follow-up`, {
              method: 'POST',
              body: { at: d.at, note: d.note || undefined },
            });
            UI.ok(r.message);
            m.close();
            done();
        ...
        const cl = m.el.querySelector('#clearFu');
        if (cl)
          cl.onclick = async () => {
            try {
              const r = await api(`/tickets/${t.id}/follow-up`, { method: 'POST', body: { at: null } });
              UI.ok(r.message);
              m.close();
              done();
            } catch (err) {
      ...
      if (act === 'edit') {
        const m = UI.modal({
          ...
          body: `<form id="edForm">${UI.field('subject', 'Assunto', UI.input('subject', t.subject, 'required'), { required: true })}<div class="form-row">${UI.field(
            'channel',
            'Canal',
            UI.select(
              'channel',
              (this.channels || [t.channel]).map((c) => [c, c]),
              t.channel,
            ),
          )}${UI.field(
            'priority',
          ...
          )}</div>${UI.field('description', 'Descrição', UI.textarea('description', t.description, 'data-type="nullable"'))}<input type="hidden" name="version" value="${t.version}" data-type="int"></form>`,
          footer: `<button class="btn secondary" data-close>Cancelar</button><button class="btn" type="submit" form="edForm">Salvar</button>`,
        });
        m.el.querySelector('#edForm').onsubmit = async (e) => {
          e.preventDefault();
          try {
            const r = await api(`/tickets/${t.id}`, { method: 'PUT', body: UI.formData(e.target) });
            UI.ok(r.message);
            m.close();
            done();
          } catch (err) {
      ...
      UI.err(err);
      if (err.status === 409) done();
    }
  },

  onRealtime() {
    if (this.id) this.detail(this.el, this.id);
    else this.list();
  },
};
