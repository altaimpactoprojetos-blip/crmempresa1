...
CRM.pages.tickets = {
  ...
  messages(r) {
    ...
    for (const e of events) {
      ...
      const atts = (byEvent[e.id] || []).map((a) => `<div class="att"><a href="${a.url || `/api/tickets/${r.ticket.id}/attachments/${a.id}`}" target="_blank" rel="noopener" ${a.url ? `download="${UI.attr(a.name)}"` : ''}>${a.mime && a.mime.startsWith('image/') ? UI.icons.image : UI.icons.file} ${UI.esc(a.name)}${a.size ? ` <span class="muted">(${UI.fmtBytes(a.size)})</span>` : ''}</a></div>`).join('');
