...
CRM.pages.pipeline = {
  ...
  async board() {
    ...
    box.innerHTML = `<div class="kanban">${r.stages
      .map((s) => {
        const items = r.opportunities.filter((o) => o.stage_id === s.id);
        const sum = items.reduce((a, o) => a + Number(o.value || 0), 0);
        ...
        ${
          items
            .map(
              (
                o,
              ) => `<div class="kb-card ${o.next_action_at && new Date(o.next_action_at) < Date.now() ? 'late' : ''}" draggable="true" data-id="${o.id}" data-version="${o.version}"><div class="title">${UI.esc(o.title)}</div><div class="small">${UI.esc(o.customer_name)}${o.customer_company ? ` · ${UI.esc(o.customer_company)}` : ''}</div>
          <div class="meta"><span class="value">${UI.fmtMoney(o.value)}</span><span>${UI.esc(o.owner_name || '—')}</span></div>${o.next_action ? `<div class="meta"><span>→ ${UI.esc(o.next_action)}</span><span>${o.next_action_at ? UI.fmtDate(o.next_action_at) : ''}</span></div>` : ''}${o.expected_close_date && s.kind === 'open' ? `<div class="meta"><span>Previsão: ${UI.fmtDate(o.expected_close_date)}</span></div>` : ''}${o.lost_reason ? `<div class="meta"><span class="badge danger">${UI.esc(o.lost_reason)}</span></div>` : ''}</div>`,
            )
            .join('') || '<div class="muted small center" style="padding:1rem">Vazio</div>'
        }</div>`;
      })
      .join('')}</div>
    ...
    box.querySelectorAll('.kb-col').forEach((col) => {
      ...
      col.ondrop = async (e) => {
        ...
        this.move(Number(d.id), Number(col.dataset.stage), col.dataset.kind, Number(d.version));
      };
    });
  },
  ...
  async move(id, stageId, kind, version) {
    try {
      let lost_reason;
      if (kind === 'lost') {
        lost_reason = await UI.prompt('Qual foi o motivo da perda?', {
          title: 'Marcar como perdida',
          placeholder: 'ex.: Preço, prazo, concorrente...',
        });
        if (lost_reason === null) return;
      }
      const r = await api(`/opportunities/${id}/move`, {
        method: 'POST',
        body: { stage_id: stageId, lost_reason, version },
      });
      UI.ok(r.message);
      this.board();
    } catch (err) {
  ...
  async form({ customer, ticket_id, opp, pipelineId } = {}, onSaved) {
    ...
    if (!customer && !isEdit)
      CRM.pages.tickets.customerPicker(
        ...
        form.querySelector('#custResults'),
        form.customer_id,
        m,
      );
    form.onsubmit = async (e) => {
      ...
      try {
        const r = isEdit
          ? await api(`/opportunities/${opp.id}`, { method: 'PUT', body: d })
          : await api('/opportunities', { method: 'POST', body: d });
        UI.ok(r.message);
        m.close();
        if (onSaved) onSaved(r.opportunity);
      } catch (err) {
  ...
  async detail(id) {
    ...
    const m = UI.modal({
      ...
      onClose: () => {
        if (location.hash.startsWith('#/funil/')) history.replaceState(null, '', '#/funil');
      },
    });
    m.el.querySelector('#editBtn').onclick = () => {
      m.close();
      this.form({ opp: o }, () => {
        this.board();
        this.detail(id);
    ...
    m.el.querySelector('#moveBtn').onclick = () => {
      const sel = m.el.querySelector('#moveSel');
      if (!sel.value) return;
      const st = openStages.find((s) => s.id === Number(sel.value));
      m.close();
      this.move(o.id, st.id, st.kind, o.version);
    };
    m.el.querySelector('#taskBtn').onclick = () =>
      CRM.pages.tasks.form(
        { customer_id: o.customer_id, customer_name: o.customer_name, opportunity_id: o.id, assignee_id: o.owner_id },
        () => {
          m.close();
          this.detail(id);
        },
      );
    const del = m.el.querySelector('#delBtn');
    if (del)
      del.onclick = async () => {
        if (
          !(await UI.confirm('Excluir esta oportunidade? Esta ação não pode ser desfeita.', {
            danger: true,
            okLabel: 'Excluir',
          }))
        )
          return;
        try {
          const r2 = await api(`/opportunities/${o.id}`, { method: 'DELETE' });
          UI.ok(r2.message);
          m.close();
          this.board();
        } catch (err) {
  ...
  onRealtime() {
    if (!document.querySelector('.modal-backdrop')) this.board();
  },
};
