...
router.post(
  ...
  async (req, res, next) => {
    try {
      if (!wa.configured)
        return next(
          new HttpError(503, 'Integração com o WhatsApp não está configurada. Registre a interação manualmente.'),
        );
      ...
      if (req.data.ticket_id) {
        await query(
          `INSERT INTO ticket_events (ticket_id, user_id, kind, direction, channel, body, payload) VALUES ($1,$2,'interaction','saida','WhatsApp',$3,$4)`,
          [
            req.data.ticket_id,
            req.user.id,
            req.data.body,
            JSON.stringify({ via: 'whatsapp_api', wa_message_id: waId }),
          ],
        );
        await query(
