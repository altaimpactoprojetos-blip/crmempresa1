...
router.get('/', async (req, res, next) => {
  try {
    ...
    const list =
      req.user.role === 'admin'
        ? rows
        : rows.map((u) => ({ id: u.id, name: u.name, role: u.role, active: u.active, available: u.available }));
    res.json({ users: list });
  } catch (err) {
    next(err);
  }
...
router.put('/:id', requireRole('admin'), validate(userSchema.partial()), async (req, res, next) => {
  try {
    ...
    await audit(req, 'user_update', 'user', id, {
      fields: Object.keys(d).filter((k) => k !== 'password'),
    ...
    try {
      ...
      const result = await tx(async (client) => {
        ...
        const pending = {
          tickets: (
            await client.query(
              `SELECT id FROM tickets WHERE assignee_id = $1 AND status NOT IN ('resolvido','cancelado')`,
              [id],
            )
          ).rows.map((r) => r.id),
          tasks: (await client.query('SELECT id FROM tasks WHERE assignee_id = $1 AND done_at IS NULL', [id])).rows.map(
            (r) => r.id,
          ),
          opportunities: (
            await client.query(
              `SELECT o.id FROM opportunities o JOIN pipeline_stages s ON s.id = o.stage_id WHERE o.owner_id = $1 AND s.kind = 'open'`,
              [id],
            )
          ).rows.map((r) => r.id),
        ...
        if (total > 0 && !target) {
          throw conflict('Este usuário possui pendências. Informe para quem transferi-las.', {
            pending: {
        ...
        if (target) {
          if (pending.tickets.length) {
            await client.query(
              `UPDATE tickets SET assignee_id = $1, status = CASE WHEN status = 'em_atendimento' THEN 'em_atendimento' ELSE status END,
            version = version + 1, updated_at = now() WHERE id = ANY($2::int[])`,
              [target, pending.tickets],
            );
            for (const tid of pending.tickets) {
              await client.query(
                ...
                [
                  tid,
                  req.user.id,
                  'Atendimento transferido por desativação de usuário',
                  JSON.stringify({ from: id, to: target, action: 'transfer' }),
                ],
              );
            }
          }
          if (pending.tasks.length)
            await client.query('UPDATE tasks SET assignee_id = $1, updated_at = now() WHERE id = ANY($2::int[])', [
              target,
              pending.tasks,
            ]);
          if (pending.opportunities.length)
            await client.query(
              'UPDATE opportunities SET owner_id = $1, version = version + 1, updated_at = now() WHERE id = ANY($2::int[])',
              [target, pending.opportunities],
            );
          if (pending.customers.length)
            await client.query('UPDATE customers SET owner_id = $1, updated_at = now() WHERE id = ANY($2::int[])', [
              target,
              pending.customers,
            ]);
        }
        await client.query('UPDATE users SET active = FALSE, available = FALSE, updated_at = now() WHERE id = $1', [
          id,
        ]);
      ...
      res.json({
        ok: true,
        message: 'Usuário desativado. O histórico foi preservado.',
        transferred: {
