...
function clean(d) {
  ...
  if (out.document) {
    if (!validDocument(out.document))
      throw badRequest('CPF ou CNPJ inválido.', { fields: { document: 'CPF/CNPJ inválido.' } });
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (req.query.q) {
      ...
      where.push(
        `(c.name ILIKE $${textIdx} OR c.email ILIKE $${textIdx} OR c.company ILIKE $${textIdx} ${digitsClause})`,
      );
    }
    if (req.query.source) {
    ...
    if (req.query.pending_followup === 'true') {
      where.push(
        `EXISTS (SELECT 1 FROM tickets t WHERE t.customer_id = c.id AND t.follow_up_at IS NOT NULL AND t.status NOT IN ('resolvido','cancelado'))`,
      );
    }
...
router.post('/import', validate(importSchema), async (req, res, next) => {
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (rec.owner) {
        const u = users.find(
          (x) => x.name.toLowerCase() === rec.owner.toLowerCase() || x.email.toLowerCase() === rec.owner.toLowerCase(),
        );
        if (u) ownerId = u.id;
        else errors.push(`Responsável "${rec.owner}" não encontrado.`);
      }
      const warnings = [];
      if ((phoneDigits && seenPhones.has(phoneDigits)) || (email && seenEmails.has(email)))
        warnings.push('Duplicado dentro do próprio arquivo.');
    ...
    await audit(req, 'customers_import', 'customer', null, {
      imported,
      skipped,
      invalid: results.length - valid.length,
    });
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const [tickets, opps, tasks, notes, wa] = await Promise.all([
      ...
      query(
        `SELECT o.*, s.name AS stage_name, s.kind AS stage_kind, u.name AS owner_name FROM opportunities o
             JOIN pipeline_stages s ON s.id = o.stage_id LEFT JOIN users u ON u.id = o.owner_id WHERE o.customer_id = $1 ORDER BY o.created_at DESC`,
        [c.id],
      ),
      query(
        `SELECT t.*, u.name AS assignee_name FROM tasks t LEFT JOIN users u ON u.id = t.assignee_id WHERE t.customer_id = $1 ORDER BY t.done_at NULLS FIRST, t.due_at`,
        [c.id],
      ),
      query(
        `SELECT n.*, u.name AS user_name FROM customer_notes n LEFT JOIN users u ON u.id = n.user_id WHERE n.customer_id = $1 ORDER BY n.created_at DESC`,
        [c.id],
      ),
      query(
...
router.put('/:id', validate(customerSchema.partial()), async (req, res, next) => {
  try {
    ...
    if (
      ...
      req.data.owner_id !== undefined &&
      req.data.owner_id !== cur.owner_id &&
      req.data.owner_id !== req.user.id &&
      cur.owner_id !== req.user.id
    ) {
    ...
    if (dups.length && !req.query.force && (d.phone !== undefined || d.email !== undefined)) {
      return next(
        conflict('Já existe outro cliente com este telefone ou e-mail.', { duplicates: dups, can_force: true }),
      );
    }
    const { rows } = await query(
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    const used = await query(
      'SELECT (SELECT count(*) FROM tickets WHERE customer_id=$1)::int AS t, (SELECT count(*) FROM opportunities WHERE customer_id=$1)::int AS o',
      [c.id],
    );
    if (used.rows[0].t || used.rows[0].o)
      return next(
        conflict(
          'Este cliente possui atendimentos ou oportunidades e não pode ser excluído. O histórico deve ser preservado.',
        ),
      );
