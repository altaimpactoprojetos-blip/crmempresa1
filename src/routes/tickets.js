...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (q.follow_up === 'pending')
      where.push(`t.follow_up_at IS NOT NULL AND t.status = ANY('{aguardando,em_atendimento,aguardando_cliente}')`);
    if (q.q) {
      params.push(`%${q.q.trim()}%`);
      where.push(
        `(t.protocol ILIKE $${params.length} OR t.subject ILIKE $${params.length} OR c.name ILIKE $${params.length} OR c.phone ILIKE $${params.length})`,
      );
    }
    if (isDateString(q.from)) {
      params.push(q.from);
      where.push(`t.opened_at >= ${startOfDaySql(`$${params.length}`)}`);
    }
    if (isDateString(q.to)) {
      params.push(q.to);
      where.push(`t.opened_at < ${endOfDaySql(`$${params.length}`)}`);
    }
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const events = await query(
      `SELECT e.*, u.name AS user_name FROM ticket_events e LEFT JOIN users u ON u.id = e.user_id WHERE e.ticket_id = $1 ORDER BY e.created_at ASC, e.id ASC`,
      [t.id],
    );
    const tasks = await query(
      `SELECT t.*, u.name AS assignee_name FROM tasks t LEFT JOIN users u ON u.id = t.assignee_id WHERE t.ticket_id = $1 ORDER BY t.due_at`,
      [t.id],
    );
    const opps = await query(
      `SELECT o.id, o.title, o.value, s.name AS stage_name, s.kind AS stage_kind FROM opportunities o JOIN pipeline_stages s ON s.id = o.stage_id WHERE o.ticket_id = $1`,
      [t.id],
    );
...
router.post('/', validate(ticketSchema), async (req, res, next) => {
  try {
    ...
    const result = await tx(async (client) => {
      ...
      const t = rows[0];
      await addEvent(client, t.id, req.user.id, 'system', 'Atendimento aberto', {
        payload: { action: 'created', channel: d.channel },
      });
      if (assigneeId) {
        await addEvent(
          client,
          ...
          req.user.id,
          'system',
          distributed ? 'Distribuído automaticamente (rodízio)' : 'Responsável atribuído',
          { payload: { action: 'assigned', to: assigneeId, auto: distributed } },
        );
      ...
      await audit(
        ...
        'ticket',
        t.id,
        { protocol, assignee_id: assigneeId, auto: distributed },
        client,
      );
      return t;
    });
...
router.put(
  ...
  async (req, res, next) => {
    try {
      const t = await loadTicket(req, Number(req.params.id));
      if (!isManager(req.user) && t.assignee_id !== req.user.id && t.assignee_id !== null)
        return next(forbidden('Somente o responsável pode editar este atendimento.'));
      const d = req.data;
      if (d.version !== undefined && d.version !== t.version)
        return next(
          conflict('Este atendimento foi alterado por outro usuário. Recarregue para ver a versão atual.', {
            current: t,
          }),
        );
      const { rows } = await query(
    ...
    const result = await tx(async (client) => {
      ...
      if (!upd.rowCount) {
        const cur = await client.query(
          `SELECT t.status, t.assignee_id, u.name AS assignee_name FROM tickets t LEFT JOIN users u ON u.id = t.assignee_id WHERE t.id = $1`,
          [id],
        );
        if (!cur.rowCount) throw notFound('Atendimento não encontrado.');
        const c = cur.rows[0];
        if (c.assignee_id && c.assignee_id !== req.user.id)
          throw conflict(`Este atendimento já foi assumido por ${c.assignee_name}.`, {
            assignee_id: c.assignee_id,
            status: c.status,
          });
        throw conflict(`Este atendimento não está aguardando (status atual: ${STATUS_LABEL[c.status]}).`, {
          status: c.status,
        });
      }
      await addEvent(client, id, req.user.id, 'system', 'Atendimento assumido', {
        payload: { action: 'claimed', to: req.user.id },
      });
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await tx(async (client) => {
        ...
        if (!isManager(req.user) && t.assignee_id !== req.user.id && t.assignee_id !== null)
          throw forbidden('Somente o responsável ou um supervisor pode transferir este atendimento.');
        if (!OPEN.includes(t.status))
          throw conflict('Não é possível transferir um atendimento encerrado. Reabra-o primeiro.');
        if (req.data.version !== undefined && req.data.version !== t.version)
          throw conflict('Este atendimento foi alterado por outro usuário. Recarregue para ver a versão atual.', {
            current: t,
          });
...
router.post('/:id/release', async (req, res, next) => {
  try {
    ...
    const result = await tx(async (client) => {
      const t = await loadTicket(req, id, client);
      if (!isManager(req.user) && t.assignee_id !== req.user.id)
        throw forbidden('Somente o responsável ou um supervisor pode devolver este atendimento à fila.');
      if (!OPEN.includes(t.status)) throw conflict('Atendimento encerrado não pode voltar à fila. Reabra-o primeiro.');
      const upd = await client.query(
        `UPDATE tickets SET assignee_id = NULL, status = 'aguardando', version = version + 1, updated_at = now() WHERE id = $1 RETURNING *`,
        [id],
      );
      await addEvent(client, id, req.user.id, 'system', 'Devolvido à fila de espera', {
        payload: { action: 'released', from: t.assignee_id },
      });
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await tx(async (client) => {
        const t = await loadTicket(req, id, client);
        if (!isManager(req.user) && t.assignee_id !== req.user.id)
          throw forbidden('Somente o responsável ou um supervisor pode alterar o status.');
        if (!OPEN.includes(t.status)) throw conflict('Atendimento já encerrado. Use "Reabrir" para continuar.');
        if (req.data.version !== undefined && req.data.version !== t.version)
          throw conflict('Este atendimento foi alterado por outro usuário. Recarregue para ver a versão atual.', {
            current: t,
          });
        if (!t.assignee_id && req.data.status !== 'cancelado')
          throw conflict('Assuma o atendimento antes de alterar o status.');
...
router.post('/:id/reopen', validate(z.object({ note: z.string().max(2000).optional() })), async (req, res, next) => {
  try {
    ...
    const result = await tx(async (client) => {
      ...
      const assigneeActive = t.assignee_id
        ? (await client.query('SELECT 1 FROM users WHERE id = $1 AND active', [t.assignee_id])).rowCount > 0
        : false;
      ...
      await addEvent(
        ...
        req.user.id,
        'system',
        `Atendimento reaberto${req.data.note ? ` — ${req.data.note}` : ''}`,
        { payload: { action: 'reopened', from: t.status, to: status } },
      );
      if (assigneeActive && t.assignee_id !== req.user.id)
        await notify(
          t.assignee_id,
  ...
  async (req, res, next) => {
    try {
      ...
      const out = await tx(async (client) => {
        const t = await loadTicket(req, id, client);
        if (!isManager(req.user) && t.assignee_id !== req.user.id)
          throw forbidden('Assuma o atendimento para registrar interações.');
        const e = await addEvent(client, id, req.user.id, 'interaction', req.data.body, {
...
router.post(
  '/:id/follow-up',
  validate(z.object({ at: z.string().datetime({ offset: true }).nullable(), note: z.string().max(500).optional() })),
  async (req, res, next) => {
    try {
      const id = Number(req.params.id);
      const out = await tx(async (client) => {
        const t = await loadTicket(req, id, client);
        if (!isManager(req.user) && t.assignee_id !== req.user.id)
          throw forbidden('Somente o responsável pode agendar retorno.');
        const upd = await client.query(
          'UPDATE tickets SET follow_up_at = $1, updated_at = now() WHERE id = $2 RETURNING *',
          [req.data.at, id],
        );
        let task = null;
        if (req.data.at) {
          ...
          await addEvent(
            ...
            req.user.id,
            'system',
            `Retorno agendado para ${new Date(req.data.at).toLocaleString('pt-BR')}${req.data.note ? ` — ${req.data.note}` : ''}`,
            { payload: { action: 'follow_up', at: req.data.at } },
          );
        } else {
          await addEvent(client, id, req.user.id, 'system', 'Retorno agendado removido', {
            payload: { action: 'follow_up_cleared' },
          });
        }
...
router.post('/distribute', requireRole('admin', 'supervisor'), async (req, res, next) => {
  try {
    const out = await tx(async (client) => {
      const queue =
        await client.query(`SELECT id, protocol, subject FROM tickets WHERE status = 'aguardando' AND assignee_id IS NULL ORDER BY
      ...
      for (const t of queue.rows) {
        ...
        if (!pick) break;
        await client.query(
          'UPDATE tickets SET assignee_id = $1, version = version + 1, updated_at = now() WHERE id = $2',
          [pick.id, t.id],
        );
        await addEvent(client, t.id, req.user.id, 'system', `Distribuído automaticamente (rodízio) para ${pick.name}`, {
          payload: { action: 'assigned', to: pick.id, auto: true },
        });
        await notify(
          pick.id,
          'Novo atendimento atribuído',
      ...
      await audit(
        ...
        'ticket',
        null,
        { assigned, remaining: queue.rowCount - assigned },
        client,
      );
    ...
    const msg =
      out.assigned === 0
        ? out.remaining === 0
          ...
          : 'Nenhum atendente disponível. Os atendimentos permanecem na fila.'
