...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (req.query.q) {
      ...
      where.push(
        `(o.title ILIKE $${params.length} OR c.name ILIKE $${params.length} OR c.company ILIKE $${params.length})`,
      );
    }
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const events = await query(
      `SELECT e.*, u.name AS user_name FROM opportunity_events e LEFT JOIN users u ON u.id = e.user_id WHERE e.opportunity_id = $1 ORDER BY e.created_at`,
      [o.id],
    );
    const tasks = await query(
      `SELECT t.*, u.name AS assignee_name FROM tasks t LEFT JOIN users u ON u.id = t.assignee_id WHERE t.opportunity_id = $1 ORDER BY t.due_at`,
      [o.id],
    );
...
router.post('/', validate(schema), async (req, res, next) => {
  try {
    const d = req.data;
    if (req.user.role === 'atendente' && d.owner_id && d.owner_id !== req.user.id)
      return next(forbidden('Atendentes só podem criar oportunidades sob sua própria responsabilidade.'));
    const out = await tx(async (client) => {
      ...
      await client.query(
        ...
        [
          rows[0].id,
          req.user.id,
          `Oportunidade criada na etapa "${stage.name}"`,
          JSON.stringify({ action: 'created', stage_id: stageId }),
        ],
      );
...
router.put('/:id', validate(schema.partial()), async (req, res, next) => {
  try {
    const o = await load(req, Number(req.params.id));
    const d = req.data;
    if (!isManager(req.user) && o.owner_id !== req.user.id && o.owner_id !== null)
      return next(forbidden('Somente o responsável pode editar esta oportunidade.'));
    if (d.version !== undefined && d.version !== o.version)
      return next(
        conflict('Esta oportunidade foi alterada por outro usuário. Recarregue para ver a versão atual.', {
          current: o,
        }),
      );
    ...
    const { rows } = await query(
      ...
      [
        ...
        d.owner_id ?? null,
        d.value ?? null,
        d.next_action !== undefined,
        d.next_action ?? null,
        d.next_action_at !== undefined,
        d.next_action_at ?? null,
    ...
    if (d.owner_id !== undefined && d.owner_id !== o.owner_id && d.owner_id)
      await notify(d.owner_id, 'Oportunidade atribuída a você', o.title, `#/funil/${o.id}`);
  ...
  async (req, res, next) => {
    try {
      ...
      const out = await tx(async (client) => {
        ...
        if (!isManager(req.user) && o.owner_id !== req.user.id && o.owner_id !== null)
          throw forbidden('Somente o responsável pode mover esta oportunidade.');
        if (req.data.version !== undefined && req.data.version !== o.version)
          throw conflict('Esta oportunidade foi alterada por outro usuário. Recarregue para ver a versão atual.', {
            current: o,
          });
        const stage = (
          await client.query('SELECT * FROM pipeline_stages WHERE id = $1 AND active', [req.data.stage_id])
        ).rows[0];
        if (!stage) throw badRequest('Etapa inválida.');
        if (stage.id === o.stage_id) return o;
        if (stage.kind === 'lost' && !req.data.lost_reason)
          throw badRequest('Informe o motivo da perda.', {
            fields: { lost_reason: 'Motivo obrigatório.' },
        ...
        await client.query(
          ...
          [
            ...
            JSON.stringify({
              ...
              from: o.stage_id,
              to: stage.id,
              lost_reason: req.data.lost_reason || null,
            }),
          ],
        );
        await audit(
...
router.delete('/:id', async (req, res, next) => {
  try {
    if (!isManager(req.user))
      return next(forbidden('Apenas administradores e supervisores podem excluir oportunidades.'));
    const o = await load(req, Number(req.params.id));
