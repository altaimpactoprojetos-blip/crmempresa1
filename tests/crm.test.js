...
before(async () => {
  ...
  at1 = client(base);
  at2 = client(base);
  await admin.login('admin@t.com', 'Senha12345');
  await sup.login('sup@t.com', 'Senha12345');
  await at1.login('a1@t.com', 'Senha12345');
  await at2.login('a2@t.com', 'Senha12345');
});
after(async () => {
  server.close();
  await pool.end();
...
test('fluxo completo: cliente → atendimento → assumir → interação → oportunidade → retorno → transferir → encerrar', async () => {
  // Cadastrar cliente
  let r = await at1.post('/customers', {
    name: 'Maria Silva',
    phone: '(11) 98888-7777',
    email: 'maria@ex.com',
  ...
  assert.equal(r.data.duplicates[0].id, customerId);
  // Campos obrigatórios
  r = await at1.post('/customers', { phone: '123' });
  assert.equal(r.status, 400);
  assert.equal(r.data.fields.name, 'Campo obrigatório.');
  // CPF inválido
  r = await at1.post('/customers', { name: 'Teste CPF', document: '111.111.111-11' });
  assert.equal(r.status, 400);

  // Abrir atendimento na fila (sem responsável)
  r = await at1.post('/tickets', {
  ...
  assert.match(r.data.ticket.protocol, /^\d{8}-\d{6}$/);
  assert.equal(r.data.ticket.status, 'aguardando');
  assert.equal(r.data.ticket.assignee_id, null);

  // Atendente não pode registrar interação sem assumir
  r = await at1.post(`/tickets/${ticketId}/interactions`, { direction: 'saida', channel: 'WhatsApp', body: 'Olá' });
  assert.equal(r.status, 403);

  // Assumir (atribuir responsável)
  r = await at1.post(`/tickets/${ticketId}/claim`);
  assert.equal(r.status, 200);
  assert.equal(r.data.ticket.status, 'em_atendimento');

  // Registrar interação de saída => primeira resposta
  r = await at1.post(`/tickets/${ticketId}/interactions`, {
    direction: 'saida',
    channel: 'WhatsApp',
    body: 'Olá Maria, vou verificar seu pedido.',
  });
  assert.equal(r.status, 201);
  assert.ok(r.data.ticket.first_response_at);
  r = await at1.post(`/tickets/${ticketId}/notes`, { body: 'Cliente parece insatisfeita com o prazo.' });
  assert.equal(r.status, 201);

  // Criar oportunidade vinculada
  r = await at1.post('/opportunities', {
    title: 'Encomenda mensal',
    customer_id: customerId,
    value: 1500.5,
    ticket_id: ticketId,
    next_action: 'Enviar proposta',
    expected_close_date: '2030-01-15',
  });
  assert.equal(r.status, 201, JSON.stringify(r.data));
  oppId = r.data.opportunity.id;
  ...
  assert.equal(r.status, 200);
  assert.ok(r.data.task);
  assert.equal(r.data.task.assignee_id, 3);
  const tasks = await at1.get('/tasks?view=upcoming');
  assert.equal(tasks.data.tasks.length, 1);
  const custPending = await at1.get('/customers?pending_followup=true');
  assert.equal(custPending.data.customers.length, 1);
  ...
  assert.equal(r.status, 200);
  assert.equal(r.data.ticket.assignee_id, 4);
  // Versão antiga agora conflita
  r = await sup.post(`/tickets/${ticketId}/transfer`, { to_user_id: 3, version: cur.version });
  assert.equal(r.status, 409);
  // Atendente 1 perdeu o acesso (não é mais responsável e não está na fila)
  r = await at1.get(`/tickets/${ticketId}`);
  assert.equal(r.status, 404);
  // Atendente 2 recebeu notificação
  const notif = await at2.get('/notifications');
  assert.ok(notif.data.notifications.some((n) => n.title.includes('transferido')));

  // Encerrar (resolvido)
  r = await at2.post(`/tickets/${ticketId}/status`, { status: 'resolvido', note: 'Pedido localizado.' });
  assert.equal(r.status, 200);
  assert.ok(r.data.ticket.closed_at);
  // Oportunidade continua aberta (status separado da etapa comercial)
  const opp = (await at1.get(`/opportunities/${oppId}`)).data.opportunity;
  assert.equal(opp.stage_kind, 'open');
  assert.equal(opp.closed_at, null);

  // Reabrir e cancelar
  r = await at2.post(`/tickets/${ticketId}/reopen`, { note: 'Cliente retornou' });
  assert.equal(r.status, 200);
  assert.equal(r.data.ticket.status, 'em_atendimento');
  r = await at2.post(`/tickets/${ticketId}/status`, { status: 'resolvido' });
  assert.equal(r.status, 200);
...
test('funil: mover etapas, exigir motivo de perda e calcular conversão', async () => {
  ...
  const lost = stages.find((s) => s.kind === 'lost'),
    ...
    neg = stages.find((s) => s.name === 'Negociação');
  let r = await at1.post(`/opportunities/${oppId}/move`, { stage_id: neg.id });
  assert.equal(r.status, 200);
  r = await at1.post(`/opportunities/${oppId}/move`, { stage_id: lost.id });
  assert.equal(r.status, 400);
  assert.ok(r.data.requires_lost_reason);
  r = await at1.post(`/opportunities/${oppId}/move`, { stage_id: lost.id, lost_reason: 'Preço' });
  assert.equal(r.status, 200);
  assert.ok(r.data.opportunity.closed_at);
  const o2 = await at1.post('/opportunities', { title: 'Outra', customer_id: customerId, value: 300 });
  r = await at1.post(`/opportunities/${o2.data.opportunity.id}/move`, { stage_id: won.id });
  assert.equal(r.status, 200);
  const rep = (await sup.get('/reports/summary')).data;
  assert.equal(rep.opportunities.won, 1);
  assert.equal(rep.opportunities.lost, 1);
  assert.equal(rep.opportunities.conversion, 0.5);
  assert.equal(rep.opportunities.lost_reasons[0].reason, 'Preço');
  assert.ok(rep.tickets.avg_first_response_s >= 0);
  assert.ok(rep.tickets.avg_resolution_s >= 0);
  assert.equal(rep.tickets.resolved, 1);
  assert.ok(rep.methodology.conversion);
  // "Hoje" no fuso da empresa (padrão America/Sao_Paulo), não em UTC.
  const today = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Sao_Paulo' }).format(new Date());
  const filtered = await sup.get(
    `/reports/summary?from=${today}&to=${today}&assignee_id=3&source=WhatsApp&channel=WhatsApp`,
  );
  assert.equal(filtered.status, 200, JSON.stringify(filtered.data));
  assert.equal(filtered.data.opportunities.won, 1);
  const onlyFrom = await sup.get(`/reports/summary?from=${today}`);
  assert.equal(onlyFrom.status, 200);
  const csv = await sup.raw(`/reports/export.csv?from=${today}&to=${today}`);
  assert.equal(csv.status, 200);
  const future = (await sup.get('/reports/summary?from=2040-01-01&to=2040-01-02')).data;
  assert.equal(future.tickets.opened, 0);
});
...
test('disputa: duas pessoas tentam assumir o mesmo atendimento ao mesmo tempo', async () => {
  ...
  const ok = results.filter((r) => r.status === 200),
    conflict = results.filter((r) => r.status === 409);
  assert.equal(ok.length, 1);
  assert.equal(conflict.length, 1);
  assert.match(conflict[0].data.error, /já foi assumido por/);
  const final = (await sup.get(`/tickets/${id}`)).data.ticket;
  assert.equal(final.assignee_id, ok[0].data.ticket.assignee_id);
  // Repetição em série também é bloqueada
  const again = await (final.assignee_id === 3 ? at2 : at1).post(`/tickets/${id}/claim`);
...
test('distribuição automática em rodízio e fila quando ninguém está disponível', async () => {
  ...
  await at1.put('/auth/me/availability', { available: false });
  await at2.put('/auth/me/availability', { available: false });
  const c = await sup.post('/tickets', { customer_id: customerId, subject: 'Auto 3', channel: 'E-mail' });
  assert.equal(c.data.ticket.assignee_id, null);
  assert.equal(c.data.ticket.status, 'aguardando');
  const dist = await sup.post('/tickets/distribute');
  assert.equal(dist.data.assigned, 0);
  assert.match(dist.data.message, /Nenhum atendente disponível/);
  await at1.put('/auth/me/availability', { available: true });
  const dist2 = await sup.post('/tickets/distribute');
  assert.equal(dist2.data.assigned, 1);
...
test('permissões: atendente não gerencia usuários/configurações nem vê atendimentos de outros', async () => {
  assert.equal(
    (await at1.post('/users', { name: 'X', email: 'x@t.com', role: 'admin', password: 'Senha12345' })).status,
    403,
  );
  assert.equal(
    (await sup.post('/users', { name: 'X', email: 'x@t.com', role: 'admin', password: 'Senha12345' })).status,
    403,
  );
  ...
  assert.equal(
    (await at1.post('/tickets', { customer_id: customerId, subject: 'Para outro', channel: 'Chat', assignee_id: 4 }))
      .status,
    403,
  );
  // Atendimento do at2 é invisível ao at1; supervisor vê tudo
  const t = await at2.post('/tickets', {
    customer_id: customerId,
    subject: 'Do dois',
    channel: 'Chat',
    assignee_id: 4,
  ...
  const rep = (await at1.get('/reports/summary')).data;
  assert.ok(rep.by_assignee.every((x) => x.assignee_id === 3 || x.name === 'Sem responsável'));
  // Lista de usuários para atendente não expõe e-mails
  const users = (await at1.get('/users')).data.users;
  assert.ok(users.every((u) => u.email === undefined));
});
...
test('usuários: desativar exige transferência de pendências e preserva histórico', async () => {
  ...
  const tmp = client(base);
  await tmp.login('temp@t.com', 'Senha12345');
  const t = await tmp.post('/tickets', {
    customer_id: customerId,
    subject: 'Pendente do temp',
    channel: 'Chat',
    assignee_id: uid,
  });
  await tmp.post(`/tickets/${t.data.ticket.id}/claim`);
  let r = await admin.post(`/users/${uid}/deactivate`, {});
  assert.equal(r.status, 409);
  assert.equal(r.data.pending.tickets, 1);
  r = await admin.post(`/users/${uid}/deactivate`, { transfer_to: 3 });
  assert.equal(r.status, 200);
  assert.equal(r.data.transferred.tickets, 1);
  assert.equal((await tmp.get('/auth/me')).status, 401, 'sessão do usuário desativado deve cair');
  const tk = (await sup.get(`/tickets/${t.data.ticket.id}`)).data;
  assert.equal(tk.ticket.assignee_id, 3);
  assert.ok(
    tk.events.some((e) => e.payload.action === 'claimed' && e.user_id === uid),
    'histórico do usuário desativado preservado',
  );
  ...
  assert.equal(
    (await admin.post('/users', { name: 'Dup', email: 'ADMIN@t.com', role: 'atendente', password: 'Senha12345' }))
      .status,
    400,
  );
...
test('recuperação de senha por link gerado pelo administrador', async () => {
  const r = await admin.post('/users/4/reset-link');
  assert.equal(r.status, 200);
  ...
  assert.equal(
    (await anon.post('/auth/reset-password', { token, password: 'NovaSenha123' })).status,
    400,
    'token não reutilizável',
  );
  await anon.login('a2@t.com', 'NovaSenha123');
  const forgot = await anon.post('/auth/forgot-password', { email: 'inexistente@t.com' });
...
test('clientes: importação CSV com prévia, erros e exportação por escopo; edição com controle de versão', async () => {
  ...
  const joao = (await sup.get('/customers?q=Souza')).data.customers[0];
  assert.equal(joao.owner_id, 3);
  assert.deepEqual(joao.tags, ['lead', 'quente']);
  ...
  assert.match(csvOut.headers.get('content-type'), /text\/csv/);
  ...
  const stale = await sup.put(`/customers/${customerId}`, { city: 'Santos', version: c.version });
...
test('tarefas: visões hoje/atrasadas/futuras e conclusão', async () => {
  const past = new Date(Date.now() - 3600e3).toISOString(),
    today = new Date(Date.now() + 60e3).toISOString();
  await at1.post('/tasks', { title: 'Atrasada', due_at: past });
  await at1.post('/tasks', { title: 'Hoje', due_at: today });
  const s = (await at1.get('/tasks?view=all')).data.summary;
  assert.ok(s.overdue >= 1);
  assert.ok(s.today >= 1);
  const over = (await at1.get('/tasks?view=overdue')).data.tasks;
  const id = over[0].id;
...
test('configurações: identidade visual, etapas do funil e auditoria', async () => {
  let r = await admin.put('/settings', {
    name: 'Empresa Teste',
    primary_color: '#123456',
    logo_data: 'data:image/png;base64,iVBORw0KGgo=',
  });
  assert.equal(r.status, 200);
  assert.equal(r.data.settings.name, 'Empresa Teste');
  ...
  r = await admin.put('/settings/stages', {
    stages: [
      ...stages.map((s) => ({ id: s.id, name: s.name, kind: s.kind })).slice(0, 4),
      { name: 'Contrato', kind: 'open' },
      ...stages.slice(4).map((s) => ({ id: s.id, name: s.name, kind: s.kind })),
    ],
  });
  assert.equal(r.status, 200);
  assert.equal(r.data.stages.length, 7);
  assert.equal(r.data.stages[4].name, 'Contrato');
  assert.equal(
    (
      await admin.put('/settings/stages', {
        stages: stages.filter((s) => s.kind !== 'won').map((s) => ({ id: s.id, name: s.name, kind: s.kind })),
      })
    ).status,
    400,
  );
  ...
  for (const a of [
    ...
    assert.ok(
      audit.some((e) => e.action === a),
      `auditoria deve conter ${a}`,
    );
});
