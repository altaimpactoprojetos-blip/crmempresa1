...
async function main() {
  ...
  const { rows } = await pool.query(
    `INSERT INTO users (name, email, password_hash, role, available) VALUES ($1,$2,$3,'admin',false) RETURNING id`,
    [name, email, hash],
  );
  await pool.query(
    `INSERT INTO audit_log (user_id, action, entity, entity_id, details) VALUES ($1,'user_create','user',$2,'{"via":"create-admin"}')`,
